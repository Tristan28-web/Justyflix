...
logger = setup_logger('bolly4u_scraper')


class Bolly4uScraper(BaseScraper):
    """
    Scraper adapter for secondary free movie & series download sites
    (e.g., Bolly4u / VegaMovies / MoviesDrive catalog mirrors).
    Fetches dual-audio, 4K, 1080p, 720p, 480p releases and hub direct download links.
    """

    def __init__(self, base_url: Optional[str] = None):
        target_url = (base_url or getattr(Config, 'BOLLY4U_BASE_URL', 'https://bolly4u.org')).rstrip('/')
        super().__init__(provider_name="Bolly4u", base_url=target_url)

    def get_total_catalog_pages(self) -> int:
        """Determines max catalog pages on secondary source."""
    ...
    def scrape_catalog_page(self, page_num: int = 1) -> List[Dict[str, Any]]:
        ...
        logger.info(f"Scraping Bolly4u catalog page {page_num}: {page_url}")
        ...
        # Select post cards or article entries
        posts = soup.select('.post, article, .item')
        if not posts:
    ...
    def scrape_movie_details(self, movie_url: str) -> Optional[Dict[str, Any]]:
        ...
        # Extract quality download links
        ...
        for a in soup.find_all('a', href=True):
            ...
            if any(k in text_lower or k in href.lower() for k in ['download', 'drive', 'hubcloud', 'gdflix', 'gdirect', '1080p', '720p', '480p', '4k']):
                ...
                quality = q_match.group(1).upper() if q_match else "720p"
        ...
        movie_record = {
            ...
            "title": clean_title,
            "year": year,
            "genre": ", ".join(list(set(genres))) if genres else "Action, Adventure, Sci-Fi",
            "rating": "7.4",
            "duration": "2h 10m",
